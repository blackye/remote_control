...
/*********************************************************************
* OpenQQwry(CStringA szFileName)
*
* 用来打开IP数据库文件的函数函数
*
* 说明:一般打开QQWry.dat文件
*********************************************************************/
bool CQQwry::OpenQQwry(CString szFileName)
{
...
* 说明:
*********************************************************************/
int CQQwry::GetStartIPInfo(int iIndex)
{
	BYTE buf[MAXBUF];
	int ioff;
	if(!m_bOpen) return 0;
...
int CQQwry::GetRecordCount(void)//得到总的IP记录数
{
	if(!m_bOpen) return 0;
...
CStringA CQQwry::GetStr(int ioffset)//根据位置读信息
{
...
* 说明:
*********************************************************************/
CStringA CQQwry::GetCountryLocal(int index)
{
...
CStringA CQQwry::GetCountryLocal(BYTE bMode,int ioffset)
{
	CStringA buf="";
	if(bMode==1)//X 没有跳
	{
...
* 说明:
*********************************************************************/
void CQQwry::SaveToFile(CStringA Name)
{
// 	CStringA str1,str2;
// 	DWORD	dwBytesWrite;
...
/*********************************************************************
* IPtoAdd(CStringA szIP)
*
* 根据IP找到地址
*
* 说明:
*********************************************************************/
CStringA CQQwry::IPtoAdd(CStringA szIP)
{
...
/*********************************************************************
* GetIndex(CStringA szIP)
*
* 根据IP找到索引
*
* 说明:
*********************************************************************/
int CQQwry::GetIndex(CStringA szIP)
{
	int index=-1;
	DWORD dwInputIP;
	DWORD dwStartIP;
...
//根据索引得到DWORD类型的IP,GetIndex(CStringA szIP)函数中用到
DWORD CQQwry::GetSIP(int index)
{
	DWORD ip;
	BYTE b[3];
	int ioff;
...
//转CStringA为DWORD,GetIndex(CStringA szIP)函数中用到
DWORD CQQwry::IPtoDWORD(CStringA szIP)
{
	DWORD iIP;
	BYTE b[4];
	CStringA szTemp;
...
void CQQwry::SetPath(CString path)
{
